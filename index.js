...
//   };
//   try{
//     const reponseBOdy = await axios.post('https://api.hubapi.com/oauth/v1/token', queryingstring.stringify(authCodeProof));
//     refreshTokenStore[req.sessionID] = reponseBOdy.data.refresh_token;
...
app.get("/review-book", async (req, res) =>{
  try{
    if(isAuthorized(req.sessionID)){
      console.log("here")
...
app.post('/review-book', async (req, res) => {
  try {
    if (isAuthorized(req.sessionID)){
      const accessToken = await getAccessToken(req.sessionID);
      const headers = {
      ...
      const body = {
        //event templateID is hardcoded
        "eventTemplateId": "1054624",
        "email": "halejasper1998@gmail.com",
        "tokens": {
      ...
      const event = `https://api.hubapi.com/integrators/timeline/v3/events`;
      try{
        const resp = await axios.post(event, body, {headers});
        res.send('thanks!');
      }catch(err){
        console.error(err);
      }
